...
<?php
	...
	foreach($this->cart->cart_products as $k => $cart_product) {
		if(empty($cart_product) || $cart_product->cart_product_quantity == 0 || !isset($this->cart->products[ $k ]))
			continue;
...
window.cartMgr.addProduct = function(el) {
	...
	var url = '<?php echo hikashop_completeLink('cart&task=addproduct&cid='.(int)$this->cart->cart_id.'&tmpl=ajax', false, true); ?>',
