...
?>
			<td><?php
				echo $this->popupHelper->display(
					...
					'HIKA_PRINT',
					hikashop_completeLink('cart&task=show&print_cart=1&cart_id='.$this->cart->cart_id, true),
					'hikashop_print_cart',
					760, 480, '', '', 'link'
				);
			?></td>
...
?>
			<td>
				<a href="<?php echo $this->checkout_url; ?>">
					<span class="icon-32-cart" title="<?php echo JText::_('PROCEED_TO_CHECKOUT'); ?>"></span> <?php echo JText::_('PROCEED_TO_CHECKOUT'); ?>
				</a>
			</td>
<?php
	}
	if($this->config->get('enable_multicart') && !$this->guest) {
?>
			<td><?php
		$dropData = array();
...
?>
<?php
	if($this->config->get('enable_wishlist') && !$this->guest) {
?>
			<td><?php
		$dropData = array();
